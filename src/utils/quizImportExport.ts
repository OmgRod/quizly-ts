...
export const downloadQuiz = (quiz: Quiz): void => {
  // Only export core quiz metadata, exclude runtime data (id, userId, createdAt, playCount)
  const quizToExport = {
    title: quiz.title,
    genre: quiz.genre,
  ...
  const jsonString = JSON.stringify(quizToExport, null, 2);
...
// Strip runtime metadata (id, userId, createdAt, playCount) that will be generated server-side
const normalizeQuiz = (quiz: any): Quiz => {
  return {
    // Keep only essential quiz metadata
    title: quiz.title,
    genre: quiz.genre,
    description: quiz.description || '',
    visibility: quiz.visibility || 'DRAFT',
    // Runtime fields will be generated by server
    id: '',
    userId: '',
    authorName: '',
    playCount: 0,
