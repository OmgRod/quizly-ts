...
const AccountSettings: React.FC<AccountSettingsProps> = ({ user, onUpdate, onDelete, onBack, onLogout }) => {
  ...
  const [showJoinDate, setShowJoinDate] = useState(user.showJoinDate ?? true);
  const [showLastOnline, setShowLastOnline] = useState(user.showLastOnline ?? true);

  const [bulkUploading, setBulkUploading] = useState(false);
  const [uploadProgress, setUploadProgress] = useState({ current: 0, total: 0 });

  // Danger Zone modals
  const [showResetQuizModal, setShowResetQuizModal] = useState(false);
  ...
  const handleResetQuizData = async () => {
    setLoading(true);
    try {
      await quizAPI.resetAllUserQuizData();
      toast.success('All quiz data reset!');
      setShowResetQuizModal(false);
    } catch (err) {
      toast.error('Failed to reset quiz data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    const updateData: any = { 
      ...
      // Insert privacy toggles in the render return, after the other toggles:
      // --- Privacy Settings Section ---
      // Add toggles for join date and last online
      // Place these inside the JSX where other privacy toggles are rendered
    };
    ...
    <div className="min-h-screen bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto px-8 py-12 animate-in fade-in slide-in-from-bottom-4 duration-500 relative z-10">
        ...
        <div className="flex gap-4 mb-8">
          <button
            className={`px-6 py-3 rounded-full font-black uppercase tracking-widest text-xs transition-all focus:outline-none ${activeTab === 'profile' ? 'bg-blue-600 text-white shadow-lg' : 'bg-white/10 text-slate-400 hover:bg-blue-600/20 hover:text-white'}`}
            onClick={() => setActiveTab('profile')}
          >
            Profile
          </button>
          <button
            className={`px-6 py-3 rounded-full font-black uppercase tracking-widest text-xs transition-all focus:outline-none ${activeTab === 'privacy' ? 'bg-blue-600 text-white shadow-lg' : 'bg-white/10 text-slate-400 hover:bg-blue-600/20 hover:text-white'}`}
            onClick={() => setActiveTab('privacy')}
          >
            Privacy
          </button>
          <button
            className={`px-6 py-3 rounded-full font-black uppercase tracking-widest text-xs transition-all focus:outline-none ${activeTab === 'danger' ? 'bg-rose-600 text-white shadow-lg' : 'bg-white/10 text-slate-400 hover:bg-rose-600/20 hover:text-white'}`}
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5">
                ...
                <button
                  ...
                  className={`relative inline-flex items-center h-7 w-14 rounded-full transition-colors overflow-hidden ${profileVisibility ? 'bg-blue-600' : 'bg-white/10'}`}
                >
                  <span className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${profileVisibility ? 'translate-x-8' : 'translate-x-1'}`} />
                </button>
              </div>
              {/* Show Quiz Stats */}
              <div className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5">
                ...
                <button
                  ...
                  className={`relative inline-flex items-center h-7 w-14 rounded-full transition-colors overflow-hidden ${showQuizStats ? 'bg-blue-600' : 'bg-white/10'}`}
                >
                  <span className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${showQuizStats ? 'translate-x-8' : 'translate-x-1'}`} />
                </button>
              </div>
              {/* Anonymous Mode */}
              <div className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5">
                ...
                <button
                  ...
                  className={`relative inline-flex items-center h-7 w-14 rounded-full transition-colors overflow-hidden ${anonymousMode ? 'bg-blue-600' : 'bg-white/10'}`}
                >
                  <span className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${anonymousMode ? 'translate-x-8' : 'translate-x-1'}`} />
                </button>
              </div>
              {/* Show Join Date */}
              <div className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5">
                <div>
                  <h4 className="font-black text-white text-sm uppercase tracking-widest mb-1">Show Join Date</h4>
                  <p className="text-slate-500 text-xs font-medium">Allow others to see when you joined</p>
                </div>
                <button
                  onClick={async () => {
                    const newValue = !showJoinDate;
                    setShowJoinDate(newValue);
                    try {
                      const response = await userAPI.updateProfile({ showJoinDate: newValue });
                      onUpdate(response.data.user);
                      toast.success(`Join date ${newValue ? 'visible' : 'hidden'}`);
                    } catch (err) {
                      setShowJoinDate(!newValue);
                      toast.error('Failed to update privacy setting');
                    }
                  }}
                  className={`relative inline-flex items-center h-7 w-14 rounded-full transition-colors overflow-hidden ${showJoinDate ? 'bg-blue-600' : 'bg-white/10'}`}
                >
                  <span className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${showJoinDate ? 'translate-x-8' : 'translate-x-1'}`} />
                </button>
              </div>
              {/* Show Last Online */}
              <div className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5">
                <div>
                  <h4 className="font-black text-white text-sm uppercase tracking-widest mb-1">Show Last Online</h4>
                  <p className="text-slate-500 text-xs font-medium">Allow others to see your last active time</p>
                </div>
                <button
                  onClick={async () => {
                    const newValue = !showLastOnline;
                    setShowLastOnline(newValue);
                    try {
                      const response = await userAPI.updateProfile({ showLastOnline: newValue });
                      onUpdate(response.data.user);
                      toast.success(`Last online ${newValue ? 'visible' : 'hidden'}`);
                    } catch (err) {
                      setShowLastOnline(!newValue);
                      toast.error('Failed to update privacy setting');
                    }
                  }}
                  className={`relative inline-flex items-center h-7 w-14 rounded-full transition-colors overflow-hidden ${showLastOnline ? 'bg-blue-600' : 'bg-white/10'}`}
                >
                  <span className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${showLastOnline ? 'translate-x-8' : 'translate-x-1'}`} />
                </button>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'danger' && (
          <div className="glass p-10 rounded-[3rem] border-rose-500/10 bg-rose-500/5 space-y-8">
            ...
            {/* Reset All Quiz Data */}
            <div className="space-y-2">
              <p className="text-slate-500 text-sm font-medium">Reset all your quiz data. This will remove all your created quizzes and quiz history, but keep your account.</p>
              <button
                onClick={() => setShowResetQuizModal(true)}
                className="text-yellow-500 font-black text-xs uppercase tracking-[0.2em] hover:text-yellow-400 transition-all pt-2 block"
              >
                Reset All Quiz Data
              </button>
            </div>
            ...
            <div className="space-y-2">
              <p className="text-slate-500 text-sm font-medium">Log out of all devices and sessions. This will end all active sessions except this one.</p>
              <button
                onClick={() => setShowLogoutAllModal(true)}
                className="text-blue-500 font-black text-xs uppercase tracking-[0.2em] hover:text-blue-400 transition-all pt-2 block"
              >
                Log Out All Sessions
              </button>
            </div>
        ...
        {/* Confirmation Modals (always rendered for all tabs) */}
        {showDeleteModal && (
        ...
        {/* Reset Quiz Data Modal */}
        {showResetQuizModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-300"></div>
            <div className="glass p-12 rounded-[3.5rem] border-yellow-500/20 w-full max-w-md relative z-10 animate-in zoom-in duration-300 text-center space-y-8">
              <div className="w-24 h-24 bg-yellow-500/10 rounded-[2rem] flex items-center justify-center text-yellow-500 text-5xl mx-auto mb-4 animate-bounce">
                <i className="bi bi-arrow-counterclockwise"></i>
              </div>
              <div className="space-y-4">
                <h3 className="text-3xl font-black text-white uppercase tracking-tighter leading-tight">Reset All Quiz Data?</h3>
                <p className="text-slate-500 text-sm font-medium">This will remove all your created quizzes and quiz history. Your account will remain.</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <button
                  onClick={handleResetQuizData}
                  disabled={loading}
                  className="bg-yellow-500 hover:bg-yellow-400 text-white font-black py-4 rounded-2xl text-xs uppercase tracking-widest transition-all shadow-xl shadow-yellow-500/20 active:scale-95"
                >
                  {loading ? "Resetting..." : "Yes, Reset"}
                </button>
                <button
                  onClick={() => setShowResetQuizModal(false)}
                  className="bg-white/5 text-slate-400 font-black py-4 rounded-2xl text-xs uppercase tracking-widest hover:text-white transition-all active:scale-95"
                >
                  Cancel
                </button>
        ...
        {/* Log Out All Sessions Modal */}
        {showLogoutAllModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-300"></div>
            <div className="glass p-12 rounded-[3.5rem] border-blue-500/20 w-full max-w-md relative z-10 animate-in zoom-in duration-300 text-center space-y-8">
              <div className="w-24 h-24 bg-blue-500/10 rounded-[2rem] flex items-center justify-center text-blue-500 text-5xl mx-auto mb-4 animate-bounce">
                <i className="bi bi-box-arrow-right"></i>
              </div>
              <div className="space-y-4">
                <h3 className="text-3xl font-black text-white uppercase tracking-tighter leading-tight">Log Out All Sessions?</h3>
                <p className="text-slate-500 text-sm font-medium">This will end all your active sessions except this one.</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <button
                  onClick={handleLogoutAllSessions}
                  disabled={loading}
                  className="bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-2xl text-xs uppercase tracking-widest transition-all shadow-xl shadow-blue-600/20 active:scale-95"
                >
                  {loading ? "Logging out..." : "Yes, Log Out All"}
                </button>
                <button
